...
func (s *BasicTestSuite) Test_Account() {
	...
	s.Require().NoError(err)
	s.T().Logf("Wait for tx: %s", hex.EncodeToString(waitForTx.Hash))
...
func (s *BasicTestSuite) Test_Payment() {
	...
	s.Require().NoError(err)
	t.Logf("Wait for tx: %s", hex.EncodeToString(waitForTx.Hash))
