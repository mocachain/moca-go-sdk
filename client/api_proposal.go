...
// SubmitProposal - Submit a proposal to Moca.
//
// - ctx: Context variables for the current API call.
//
// - msgs: The messages to be executed when the proposal is passed.
//
// - depositAmount: The amount of amoca to deposit to the proposal.
//
...
func (c *Client) SubmitProposal(ctx context.Context, msgs []sdk.Msg, depositAmount math.Int, title, summary string, opts types.SubmitProposalOptions) (uint64, string, error) {
	msgSubmitProposal, err := govTypesV1.NewMsgSubmitProposal(msgs, sdk.NewCoins(sdk.NewCoin(gnfdSdkTypes.Denom, depositAmount)), c.defaultAccount.GetAddress().String(), opts.Metadata, title, summary, false)
	if err != nil {
		return 0, "", err
	}
