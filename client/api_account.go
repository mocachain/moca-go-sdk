...
func (c *Client) GetAccount(ctx context.Context, address string) (authTypes.AccountI, error) {
	...
	baseAccount := mocadTypes.EthAccount{}
...
// Transfer - Transfer amoca from sender to receiver.
//
// - ctx: Context variables for the current API call.
//
// - toAddress: The address who will receive the amoca.
//
// - amount: The amoca amount to transfer, 1e18 denotes 1amoca.
//
// - txOption: The txOption for sending transactions.
//
...
func (c *Client) sendTransferEvmTx(ctx context.Context, to string, amount math.Int) (string, error) {
	...
	signedTx, err := evmTypes.SignTx(tx, evmTypes.NewEIP155Signer(c.evmChainID), privateKey)
	if err != nil {
		return "", err
	}
	if err = c.evmClient.SendTransaction(context.Background(), signedTx); err != nil {
		return "", err
	}
