...
type ISPClient interface {
	...
	UpdateSpStoragePrice(ctx context.Context, spAddr string, readPrice, storePrice math.LegacyDec, freeReadQuota uint64, txOption gnfdSdkTypes.TxOption) (string, error)
...
func (c *Client) CreateStorageProvider(ctx context.Context, fundingAddr, sealAddr, approvalAddr, gcAddr, maintenanceAddr, blsPubKey, blsProof, endpoint string, depositAmount math.Int, description spTypes.Description, opts types.CreateStorageProviderOptions) (uint64, string, error) {
	...
	if opts.ProposalDepositAmount.IsNil() {
		opts.ProposalDepositAmount = math.NewIntWithDecimal(1, gnfdSdkTypes.DecimalMOCA)
	}
	if opts.ReadPrice.IsNil() {
		opts.ReadPrice = math.LegacyNewDec(1)
	}
	if opts.StorePrice.IsNil() {
		opts.StorePrice = math.LegacyNewDec(1)
	}
...
// - spAddr: The HEX-encoded string of the storage provider address.
//
// - readPrice: The read price of the SP, in amoca wei per charge byte.
//
// - storePrice: The store price of the SP, in amoca wei per charge byte.
//
...
//
// - ret2: Return error when the request failed, otherwise return nil.
func (c *Client) UpdateSpStoragePrice(ctx context.Context, spAddr string, readPrice, storePrice math.LegacyDec, freeReadQuota uint64, TxOption gnfdSdkTypes.TxOption) (string, error) {
	spAcc, err := sdk.AccAddressFromHexUnsafe(spAddr)
	if err != nil {
		return "", err
	}
