...
// Client - The implementation for IClient, implement all Client APIs for Moca SDK.
type Client struct {
	// The chain Client is used to interact with the blockchain
	chainClient *sdkclient.MocaClient
	// The HTTP Client is used to send HTTP requests to the moca blockchain and sp
	...
	// The default account private key
	privateKey string
...
// New - New Moca Go SDK Client.
//
// - chainID: The Moca Blockchain's chainID that the Client would interact with.
//
// - endpoint: The Moca Blockchain's RPC URL that the Client would interact with.
//
...
func New(chainID string, endpoint, evmEndpoint, privateKey string, option Option) (IClient, error) {
	...
	evmChainID, err := evmClient.NetworkID(context.Background())
	if err != nil {
		return nil, err
	}
	c := Client{
...
func (c *Client) GetChainClient() *sdkclient.MocaClient {
...
func (c *Client) sendDelPolicyEvmTxn(ctx context.Context, msg *storageTypes.MsgDeletePolicy) (string, error) {
	session, err := c.createStorageEvmSession(ctx, c.privateKey)
	if err != nil {
		return "", err
	}
	txRsp, err := session.DeletePolicy(toStoragePrincipal(msg.Principal), msg.Resource)
	if err != nil {
		return "", err
	}
