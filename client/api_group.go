...
// CreateGroup - Create a new group without group members on Moca blockchain, and group members can be added by UpdateGroupMember transaction.
//
...
//
// # For more details regarding `Group`, please refer to https://docs.moca.network/
//
...
// DeleteGroup - Delete a group on Moca blockchain. The sender MUST only be the group owner, group members or others would fail to send this transaction.
//
...
func (c *Client) sendUpdateGroupMemberEvmTx(ctx context.Context, msg *storageTypes.MsgUpdateGroupMember, opt types.UpdateGroupMemberOption) (string, error) {
	...
	toAdd := make([]common.Address, 0, len(msg.MembersToAdd))
	expirationTime := make([]int64, 0, len(msg.MembersToAdd))
	toDel := make([]common.Address, 0, len(msg.MembersToDelete))
	for _, member := range msg.MembersToAdd {
		toAdd = append(toAdd, common.HexToAddress(member.Member))
		expirationTime = append(expirationTime, member.ExpirationTime.Unix())
	}
	for _, member := range msg.MembersToDelete {
		toDel = append(toDel, common.HexToAddress(member))
	}
	txRsp, err := session.UpdateGroup(
		common.HexToAddress(msg.GroupOwner),
		msg.GroupName,
		toAdd,
		expirationTime,
		toDel,
...
func (c *Client) PutGroupPolicy(ctx context.Context, groupName string, principalAddr string,
	...
	resource := mocadTypes.NewGroupGRN(sender, groupName)
...
func (c *Client) DeleteGroupPolicy(ctx context.Context, groupName string, principalAddr string, opt types.DeletePolicyOption) (string, error) {
	sender := c.MustGetDefaultAccount().GetAddress()
	resource := mocadTypes.NewGroupGRN(sender, groupName).String()
...
func (c *Client) RenewGroupMember(ctx context.Context, groupOwnerAddr, groupName string,
	...
	return c.sendRenewGroupMemberEvmTx(ctx, msg)
}

func (c *Client) sendRenewGroupMemberEvmTx(ctx context.Context, msg *storageTypes.MsgRenewGroupMember) (string, error) {
	...
	toRenew := make([]common.Address, 0, len(msg.Members))
	expirationTime := make([]int64, 0, len(msg.Members))
	for _, member := range msg.Members {
		toRenew = append(toRenew, common.HexToAddress(member.Member))
		expirationTime = append(expirationTime, member.ExpirationTime.Unix())
	}
