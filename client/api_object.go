...
func (c *Client) sendCreateObjectEvmTxs(ctx context.Context, msg *storageTypes.MsgCreateObject, opts types.CreateObjectOptions) (string, error) {
	...
	txRsp, err := session.CreateObject(
		...
		storage.Approval{
			...
			GlobalVirtualGroupFamilyId: msg.PrimarySpApproval.GlobalVirtualGroupFamilyId,
			Sig:                        msg.PrimarySpApproval.Sig,
		},
...
func toStorageCheckSum(s [][]byte) []string {
	e := make([]string, 0)
	for _, checksum := range s {
		e = append(e, base64.StdEncoding.EncodeToString(checksum))
	}
	return e
}

// UpdateObjectContent sends updateObjectContent tx to moca chain,
// it returns the transaction hash value and error
func (c *Client) UpdateObjectContent(ctx context.Context, bucketName, objectName string,
	...
	return c.sendUpdateObjectContentTx(ctx, updateObjectContentMsg, opts)
}

func (c *Client) sendUpdateObjectContentTx(ctx context.Context, msg *storageTypes.MsgUpdateObjectContent, opts types.UpdateObjectOptions) (string, error) {
...
// DeleteObject - Send DeleteObject msg to moca chain and return txn hash.
//
...
// CancelCreateObject send CancelCreateObject txn to moca chain
func (c *Client) CancelCreateObject(ctx context.Context, bucketName, objectName string, opt types.CancelCreateOption) (string, error) {
	...
	return c.sendCancelCreateObjectEvmTx(ctx, cancelCreateMsg)
}

func (c *Client) sendCancelCreateObjectEvmTx(ctx context.Context, msg *storageTypes.MsgCancelCreateObject) (string, error) {
...
const printRateInterval = time.Second / 2
...
func (c *Client) FGetObjectResumable(ctx context.Context, bucketName, objectName, filePath string, opts types.GetObjectOptions) error {
	...
	log.Debug().Msg(fmt.Sprintf("get object resumable begin segment Range: %s, startOffset: %d, endOffset:%d", opts.Range, startOffset, endOffset))

	pw := &ProgressWriter{
		Writer:      fd,
		Total:       int64(meta.ObjectInfo.GetPayloadSize()),
		Current:     startOffset,
		StartTime:   time.Now(),
...
func (c *Client) PutObjectPolicy(ctx context.Context, bucketName, objectName string, principalStr types.Principal,
	...
	resource := mocadTypes.NewObjectGRN(bucketName, objectName)
	...
	if opt.PolicyExpireTime != nil {
		expireTime = opt.PolicyExpireTime
	} else {
		expireTime = &storageTypes.MaxTimeStamp
	}
...
// CreateFolder send create empty object txn to moca chain
...
// DelegateCreateFolder send create empty object txn to moca chain
