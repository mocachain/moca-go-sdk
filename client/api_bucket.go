...
//
// - createBucketMsg: The msg of create bucket which defined by the moca chain.
//
...
// CreateBucket - Create a new bucket in moca.
//
// This API sends a request to the storage provider to get approval for creating  bucket and sends the createBucket transaction to the Moca.
//
// - ctx: Context variables for the current API call.
//
...
func (c *Client) createStorageEvmSession(ctx context.Context, privKey string) (*storage.IStorageSession, error) {
	...
	session, err := CreateStorageSession(c.evmClient, *txOpts, mocadTypes.StorageAddress)
	if err != nil {
		return nil, err
	}
	return session, nil
...
func toStorageApproval(a *gnfdCommonTypes.Approval) storage.Approval {
	return storage.Approval{
		ExpiredHeight:              a.ExpiredHeight,
		GlobalVirtualGroupFamilyId: a.GlobalVirtualGroupFamilyId,
		Sig:                        a.Sig,
	}
}

func (c *Client) sendCreateBucketEvmTxn(ctx context.Context, createBucketMsg *storageTypes.MsgCreateBucket, opts types.CreateBucketOptions) (string, error) {
	session, err := c.createStorageEvmSession(ctx, c.privateKey)
	if err != nil {
		return "", err
	}
	createBucketTxRsp, err := session.CreateBucket(
		...
		toStorageApproval(createBucketMsg.PrimarySpApproval),
	...
	if opts.Tags != nil {
		...
		if _, err = c.sendSetTagEvmTxn(ctx, msgSetTag); err != nil {
			return "", err
		}
	}
	return createBucketTxRsp.Hash().String(), nil
}

// DeleteBucket - Send DeleteBucket msg to moca chain and return txn hash.
//
// - ctx: Context variables for the current API call.
//
...
func (c *Client) sendDeleteBucketEvmTx(ctx context.Context, delBucketMsg *storageTypes.MsgDeleteBucket) (string, error) {
	...
	txRsp, err := session.DeleteBucket(
		delBucketMsg.BucketName,
	)
	if err != nil {
		return "", err
	}
...
//
// - visibility: The VisibilityType defines on moca which can be PUBLIC_READ, PRIVATE or INHERIT
//
// - opt: The Options for customizing the transaction.
//
// - ret1: Transaction hash return from blockchain.
...
// UpdateBucketInfo - Update the bucket meta on chain, including read quota, payment address or visibility. It will send the MsgUpdateBucketInfo msg to moca to update the meta.
//
// - ctx: Context variables for the current API call.
//
// - bucketName: The name of the bucket to be updated.
...
func (c *Client) sendUpdateBucketInfoEvmTxn(ctx context.Context, msg *storageTypes.MsgUpdateBucketInfo, opt *gnfdSdkTypes.TxOption) (string, error) {
	...
	txRsp, err := session.UpdateBucketInfo(
		...
		common.HexToAddress(msg.PaymentAddress),
		big.NewInt(int64(msg.ChargedReadQuota.Value)),
	)
	if err != nil {
		return "", err
	}
...
//
// - principalStr: Indicates the marshaled principal content of moca permission types, users can generate it by NewPrincipalWithAccount or NewPrincipalWithGroupId method.
//
...
func (c *Client) PutBucketPolicy(ctx context.Context, bucketName string, principalStr types.Principal,
	...
	resource := mocadTypes.NewBucketGRN(bucketName)
...
func toStoragePrincipal(principal *permTypes.Principal) storage.Principal {
	return storage.Principal{
		PrincipalType: int32(principal.Type),
		Value:         principal.Value,
	}
}

func toStorageStatement(statement *permTypes.Statement) storage.Statement {
	s := storage.Statement{
		Effect:    int32(statement.Effect),
		Actions:   convertActions(statement.Actions),
		Resources: statement.Resources,
	}
	if statement.ExpirationTime != nil {
		s.ExpirationTime = statement.ExpirationTime.Unix()
	}
	if statement.LimitSize != nil {
		s.LimitSize = statement.LimitSize.GetValue()
	}
	return s
}

func toStorageStatements(statement []*permTypes.Statement) []storage.Statement {
	result := make([]storage.Statement, len(statement))
	for i, s := range statement {
		result[i] = toStorageStatement(s)
	}
	return result
}

func convertActions(actions []permTypes.ActionType) []int32 {
	result := make([]int32, len(actions))
	for i, action := range actions {
		result[i] = int32(action)
	}
	return result
}

func (c *Client) sendPutPolicyEvmTxn(ctx context.Context, msg *storageTypes.MsgPutPolicy) (string, error) {
	session, err := c.createStorageEvmSession(ctx, c.privateKey)
	if err != nil {
		return "", err
	}
	...
	if msg.ExpirationTime != nil {
		expireTime = msg.ExpirationTime.Unix()
	}
	txRsp, err := session.PutPolicy(toStoragePrincipal(msg.Principal), msg.Resource, toStorageStatements(msg.Statements), expireTime)
	if err != nil {
		return "", err
	}
	return txRsp.Hash().String(), nil
...
func (c *Client) DeleteBucketPolicy(ctx context.Context, bucketName string, principalStr types.Principal, opt types.DeletePolicyOption) (string, error) {
	resource := mocadTypes.NewBucketGRN(bucketName).String()
	principal := &permTypes.Principal{}
	if err := principal.Unmarshal([]byte(principalStr)); err != nil {
		return "", err
	}
...
//
// - ret1: The bucket policy info defined on moca.
//
// - ret2: Return error when the request failed, otherwise return nil.
...
// - ctx: Context variables for the current API call.
//
// - migrateBucketMsg: Indicates msg of migrating bucket which defined by moca
//
...
func (c *Client) sendMigrateBucketEvmTX(ctx context.Context, msg *storageTypes.MsgMigrateBucket) (string, error) {
	...
	txRsp, err := session.MigrateBucket(
		msg.BucketName,
		msg.DstPrimarySpId,
		toStorageApproval(msg.DstPrimarySpApproval),
	)
	if err != nil {
		return "", err
	}
